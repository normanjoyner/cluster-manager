/*
Copyright The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	v3 "github.com/containership/cluster-manager/pkg/apis/auth.containership.io/v3"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeAuthorizationRoleBindings implements AuthorizationRoleBindingInterface
type FakeAuthorizationRoleBindings struct {
	Fake *FakeContainershipAuthV3
	ns   string
}

var authorizationrolebindingsResource = schema.GroupVersionResource{Group: "auth.containership.io", Version: "v3", Resource: "authorizationrolebindings"}

var authorizationrolebindingsKind = schema.GroupVersionKind{Group: "auth.containership.io", Version: "v3", Kind: "AuthorizationRoleBinding"}

// Get takes name of the authorizationRoleBinding, and returns the corresponding authorizationRoleBinding object, and an error if there is any.
func (c *FakeAuthorizationRoleBindings) Get(name string, options v1.GetOptions) (result *v3.AuthorizationRoleBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(authorizationrolebindingsResource, c.ns, name), &v3.AuthorizationRoleBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v3.AuthorizationRoleBinding), err
}

// List takes label and field selectors, and returns the list of AuthorizationRoleBindings that match those selectors.
func (c *FakeAuthorizationRoleBindings) List(opts v1.ListOptions) (result *v3.AuthorizationRoleBindingList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(authorizationrolebindingsResource, authorizationrolebindingsKind, c.ns, opts), &v3.AuthorizationRoleBindingList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v3.AuthorizationRoleBindingList{ListMeta: obj.(*v3.AuthorizationRoleBindingList).ListMeta}
	for _, item := range obj.(*v3.AuthorizationRoleBindingList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested authorizationRoleBindings.
func (c *FakeAuthorizationRoleBindings) Watch(opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(authorizationrolebindingsResource, c.ns, opts))

}

// Create takes the representation of a authorizationRoleBinding and creates it.  Returns the server's representation of the authorizationRoleBinding, and an error, if there is any.
func (c *FakeAuthorizationRoleBindings) Create(authorizationRoleBinding *v3.AuthorizationRoleBinding) (result *v3.AuthorizationRoleBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(authorizationrolebindingsResource, c.ns, authorizationRoleBinding), &v3.AuthorizationRoleBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v3.AuthorizationRoleBinding), err
}

// Update takes the representation of a authorizationRoleBinding and updates it. Returns the server's representation of the authorizationRoleBinding, and an error, if there is any.
func (c *FakeAuthorizationRoleBindings) Update(authorizationRoleBinding *v3.AuthorizationRoleBinding) (result *v3.AuthorizationRoleBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(authorizationrolebindingsResource, c.ns, authorizationRoleBinding), &v3.AuthorizationRoleBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v3.AuthorizationRoleBinding), err
}

// Delete takes name of the authorizationRoleBinding and deletes it. Returns an error if one occurs.
func (c *FakeAuthorizationRoleBindings) Delete(name string, options *v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(authorizationrolebindingsResource, c.ns, name), &v3.AuthorizationRoleBinding{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeAuthorizationRoleBindings) DeleteCollection(options *v1.DeleteOptions, listOptions v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(authorizationrolebindingsResource, c.ns, listOptions)

	_, err := c.Fake.Invokes(action, &v3.AuthorizationRoleBindingList{})
	return err
}

// Patch applies the patch and returns the patched authorizationRoleBinding.
func (c *FakeAuthorizationRoleBindings) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v3.AuthorizationRoleBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(authorizationrolebindingsResource, c.ns, name, pt, data, subresources...), &v3.AuthorizationRoleBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v3.AuthorizationRoleBinding), err
}
